...
const UpdateLecture = () => {
  ...
  // When lecture is selected, prefill form with existing data
  useEffect(() => {
    if (selectedLectureId) {
      ...
      if (selectedLecture) {
        setFormData({ name: selectedLecture.name, file: null });
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Selected File:", formData.file);
      console.log("Selected File:", updateData.file);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white p-6">
      <div className="bg-gray-800 shadow-lg rounded-2xl p-6 w-full max-w-lg">
        <h2 className="text-2xl font-bold mb-4 text-center">Edit Lecture</h2>
